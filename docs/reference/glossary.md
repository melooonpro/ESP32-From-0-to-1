...
这些缩写不用一次背完。先把它们当成普通能力：接按钮、调亮度、读传感器、联网、保存设置、升级系统。等你在实验里见过现象，再回来看名字会轻松很多。

## ESP32

乐鑫的一系列芯片。用人话说，它是一颗能运行程序、连接外设、部分型号还能连 Wi-Fi/蓝牙的小电脑，常用于 IoT、HMI、音频、边缘 AI 等场景。不同型号的外设、内存和无线能力会有差异，写代码前要确认目标芯片。

## ESP-IDF

Espressif IoT Development Framework，乐鑫官方开发框架。它是“写 ESP32 官方程序的一整套工具箱”，包含驱动、协议栈、构建系统、示例和文档。
...
实时操作系统。用人话说，它帮一个小设备安排多件事谁先做、谁等一等。ESP-IDF 应用运行在 FreeRTOS 上，任务、队列、互斥锁、定时器都是常用概念。
