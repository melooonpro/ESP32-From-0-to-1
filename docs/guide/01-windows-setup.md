...
这一章不是为了炫工具，而是为了打通“电脑里的代码”到“手上那块板子真的动起来”的通道。

环境搭建的目标只有一个：你能在 Windows 终端里执行 `idf.py build`，并把程序烧录到 ESP32 开发板。先不要急着理解所有工具，先知道它们的分工。
...
5. 用 USB 数据线连接 ESP32 开发板，在设备管理器中确认串口号。
...
以常见 DevKit 开发板为例，连接方式通常是 USB 数据线直连开发板。注意“能充电”的线不一定能传数据。如果烧录时一直连接失败，优先换数据线、换 USB 口，再检查 BOOT/RESET 按键。
...
我正在 Windows 上安装 ESP-IDF 并连接 ESP32 开发板。请根据我贴出的终端输出、设备管理器串口信息和烧录日志，判断环境是否正确，并按优先级给出排查步骤和下一步最小验证命令。
</div>
