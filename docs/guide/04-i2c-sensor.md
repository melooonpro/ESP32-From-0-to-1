...
我有一个 I2C 传感器模块，接到 ESP32-S3 后扫描不到地址。请按硬件、电源、引脚、上拉、代码配置、地址冲突的顺序帮我排查，并说明每一步怎么验证。
</div>
...
我会贴出一个 I2C 传感器数据手册中的寄存器表。请帮我提取设备地址、初始化步骤、读取数据寄存器的流程，并生成 ESP-IDF 读数函数。
</div>
...
请基于 ESP-IDF 新版 I2C master driver 写一个“扫描 I2C 地址并打印表格”的 demo，要求 SDA/SCL 可通过 menuconfig 修改，扫描结果用十六进制输出。
</div>
